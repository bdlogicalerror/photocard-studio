          ...
          <div className="flex-shrink-0 px-6 py-4 bg-zinc-950/50 border-b border-zinc-800 flex items-center justify-between">
            ...
            <div className="flex flex-col items-end gap-1.5">
               <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-bold text-red-500">{Math.min(currentIndex, news.length)} / {news.length}</span>
               </div>
               <div className="w-32 md:w-48 h-1.5 bg-zinc-800 rounded-full overflow-hidden">
                  <div 
                    className="h-full bg-red-600 transition-all duration-500 ease-out"
                    style={{ width: `${news.length > 0 ? (Math.min(currentIndex, news.length) / news.length) * 100 : 0}%` }}
                  />
               </div>
            </div>
          </div>
